...
export default function PaymentModal({ children }: PaymentModalProps) {
  ...
  const [localPaymentStep, setLocalPaymentStep] = useState<
    "select" | "qr" | "upload"
  >("select");
  const [globalPaymentStep, setGlobalPaymentStep] = useState<
    "select" | "card" | "paypal"
  >("select");
  ...
  const handleSupportChat = useCallback(() => {
    ...
    alert(
      "Payment Assistant: မင်္ဂလာပါ! ငွေပေးချေမှုနဲ့ ပတ်သက်ပြီး ဘာကူညီပေးရမလဲ? \n\nHello! How can I help you with your payment?",
    );
  }, []);
        ...
        <div className="fixed inset-0 flex items-center justify-center p-3 sm:p-4">
          <Dialog.Panel className="w-full max-w-md">
            ...
            >
              ...
              <motion.div
                layout
                className="p-4 border-b border-gray-100 flex justify-between items-center bg-linear-to-r from-gray-50 to-white shrink-0"
              >
              ...
              <motion.div
                layout
                className="p-4 sm:p-5 bg-linear-to-br from-white to-gray-50/30 flex-1 overflow-y-auto"
              >
                <AnimatePresence mode="wait">
                  {!activeMethod ? (
