...
export function isApiError(error: unknown): error is ApiError {
  ...
  return (
    'error' in potentialError &&
    typeof potentialError.error === 'string'
  )
}
...
export function useApiErrorHandler() {
  ...
  const handleApiError = useCallback((
    ...
    if (options?.showToast !== false && shouldShow) {
      ...
      const formattedContext = context.replace(/_/g, ' ').replace(/\b\w/g, l => l.toUpperCase())
      showToast(
        `${formattedContext}: ${finalMessage}`,
        'error'
      )
    }
    ...
    return {
      ...
      error: errorObject,
      shouldRetry: options?.allowRetry !== false && 
                   (code === 'NETWORK_ERROR' || 
                    code === 'TIMEOUT_ERROR' || 
  ...
  const handleNetworkError = useCallback((error: unknown, context: string = 'Network request') => {
  ...
  const handleValidationError = useCallback((errors: Record<string, string[]>, context: string = 'Validation') => {
  ...
  const withErrorHandling = useCallback(async <T>(
    ...
    setIsLoading(false)
    return { data: null, error: { error: 'Operation failed after retries', code: 'RETRY_FAILED' } }
...
export async function safeFetch<T = any>(
...
): Promise<{ data: T | null; error: ApiError | null; response: Response | null; metadata: { attempts: number; duration: number; fromCache?: boolean } }> {
  ...
  if (options?.cacheKey && (options.cacheTtl ?? 0) > 0) {
    ...
    if (cached !== null) {
      return {
        ...
        error: null,
        response: null,
        metadata: { attempts: 0, duration: Date.now() - startTime, fromCache: true },
      }
    }
  }
      ...
      if (!response.ok) {
        ...
        try {
          ...
          errorData = isApiError(errorResponse)
            ? errorResponse
            : {
                error: errorResponse.error || `HTTP ${response.status}: ${response.statusText}`,
  ...
  console.error(`[${context}] Fetch failed after ${attempts} attempt${attempts === 1 ? '' : 's'}:`, {
    error: lastError,
    requestId,
    duration,
    url,
 ...
 */
function determineRetryStrategy(status: number, attempt: number, maxRetries: number): boolean {
 ...
 */
function shouldRetryBasedOnError(errorCode: string, attempt: number, maxRetries: number): boolean {
 ...
 */
export function createApiClient(baseUrl: string, defaultOptions?: {
  timeout?: number
  retries?: number
